...
class FavoriteController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($favourite){
            if ($favourite->delete()) {
                return response(['message' => 'Producto eliminado de favoritos','is_favorite'=>false], 200);
            }else{
        ...
        if ($favourite->save()) {
            return response(['message' => 'Producto agregado a favoritos','is_favorite'=>true], 200);
        }else{
            return response(['errors' => ['Algo salió mal']], 403);
        }
    }
