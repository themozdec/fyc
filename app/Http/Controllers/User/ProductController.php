...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($name && empty($category_id)){
            $products = Product::with('productImages')->where('active','=',true)->where('name','LIKE','%'.$name.'%')->paginate(12);
        }else if($name && !empty($category_id)){
            $products = Product::with('productImages')->where('active','=',true)->where('name','LIKE','%'.$name.'%')->where('category_id','=',$category_id)->paginate(12);
        }else if(empty($name) && !empty($category_id)){
            $products = Product::with('productImages')->where('active','=',true)->where('name','LIKE','%'.$name.'%')->where('category_id','=',$category_id)->paginate(12);
        }else{    
